// script.js – complete working script with full tours array
...
(function() {
    ...
    // 👇 IMPORTANT: Replace with your actual Vercel project URL
    const PROXY_URL = 'https://supremevip-git-main-christiantelemaque-bytes-projects.vercel.app'; // <-- UPDATE THIS
    ...
    try {
        if (STRIPE_PUBLISHABLE_KEY) {
            ...
            console.warn('Stripe publishable key not set – payment will be disabled.');
        }
    } catch (e) {
    ...
    function updateUserMenu() {
        ...
        if (currentUser) {
            ...
            menu.innerHTML = `
                <button class="btn-auth" onclick="openAuthModal()">Sign Up / Login</button>
            `;
        }
    }
    ...
    window.signup = async function(e) {
        ...
        messageDiv.innerHTML = '<div class="spinner"></div><p>Creating account...</p>';
        ...
        try {
            ...
            if (error) {
                messageDiv.innerHTML = `<p style="color: red;">Error: ${error.message}</p><button onclick="window.location.reload()" class="btn-auth">Try Again</button>`;
                setTimeout(() => {
            ...
            messageDiv.innerHTML = `<p style="color: red;">Network error: ${err.message}</p><button onclick="window.location.reload()" class="btn-auth">Try Again</button>`;
        }
    };
    ...
    window.updateTotalPrice = function() {
        ...
        document.getElementById('totalPriceDisplay').innerText = `$${total.toFixed(2)} USD`;
    };

    function initStripeElements() {
        if (stripe && elements && cardElement && !cardElement._components) {
    ...
    window.processBooking = async function(e) {
        ...
        try {
            const response = await fetch(`${PROXY_URL}/api/create-payment-intent`, {
                method: 'POST',
    ...
    window.openTourDetails = function(tourId) {
        ...
        document.getElementById('bookingLoginPrompt').style.display = currentUser ? 'none' : 'block';
        document.getElementById('bookingFormContainer').style.display = currentUser ? 'block' : 'none';

        if (currentUser) {
            setTimeout(initStripeElements, 100);
        }

        loadReviewsForTour(tourId).then(reviews => {
            const count = reviews.length;
            const avg = count ? (reviews.reduce((sum, r) => sum + r.rating, 0) / count).toFixed(1) : 0;
            document.getElementById('detail-rating-stars').innerHTML = renderStars(avg);
            document.getElementById('detail-rating-text').innerText = `(${count} ${count === 1 ? 'review' : 'reviews'})`;
        });

        document.getElementById('tourModal').style.display = 'flex';
        switchTourTab('details');
    };
    ...
    window.openTourReviews = async function(tourId) {
        ...
        if (!tour) return;
        document.getElementById('review-tour-name').innerText = tour.name[lang];
        const reviews = await loadReviewsForTour(tourId);
        updateReviewDisplay(reviews);
        document.getElementById('tourModal').style.display = 'flex';
        switchTourTab('reviews');
    };

    window.openTourBooking = function(tourId) {
        openTourDetails(tourId);
        switchTourTab('booking');
    };

    window.switchTourTab = function(tabName) {
        document.querySelectorAll('#tourModal .tab-btn').forEach(btn => btn.classList.remove('active'));
        document.querySelectorAll('#tourModal .tab-content').forEach(content => content.classList.remove('active'));
        if (tabName === 'details') {
    ...
    function updateReviewDisplay(reviews) {
        ...
        if (!reviews || reviews.length === 0) {
            container.innerHTML = `<div class="no-reviews">${i18n[lang].noReviews}</div>`;
            document.getElementById('modal-rating-stars').innerHTML = '☆☆☆☆☆';
            document.getElementById('modal-rating-text').innerText = '(0 reviews)';
            return;
        }

        const avg = (reviews.reduce((s, r) => s + r.rating, 0) / reviews.length).toFixed(1);
        document.getElementById('modal-rating-stars').innerHTML = renderStars(avg);
        document.getElementById('modal-rating-text').innerText = `(${reviews.length} ${reviews.length === 1 ? 'review' : 'reviews'})`;

        reviews.forEach(r => {
            const date = new Date(r.created_at).toLocaleDateString(lang === 'es' ? 'es-ES' : 'en-US', {
    ...
    window.submitReview = async function(event) {
        ...
        btn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> ' + i18n[lang].loading;
        btn.disabled = true;

        const result = await saveReviewToSupabase({
            tourId: currentTourId,
            name: name,
        ...
        if (result.success) {
            btn.innerHTML = '<i class="fas fa-check"></i> ' + i18n[lang].success;

            document.getElementById('reviewerName').value = '';
            document.querySelectorAll('input[name="rating"]').forEach(r => r.checked = false);
            document.getElementById('reviewText').value = '';

            const updatedReviews = await loadReviewsForTour(currentTourId);
            updateReviewDisplay(updatedReviews);

            const count = updatedReviews.length;
            const avg = count ? (updatedReviews.reduce((s, r) => s + r.rating, 0) / count).toFixed(1) : 0;
            const starsEl = document.getElementById(`rating-stars-${currentTourId}`);
            const textEl = document.getElementById(`rating-text-${currentTourId}`);
            if (starsEl) starsEl.innerText = renderStars(avg);
            if (textEl) textEl.innerText = `${avg > 0 ? avg : 'No'} ${count === 1 ? 'review' : 'reviews'}`;
